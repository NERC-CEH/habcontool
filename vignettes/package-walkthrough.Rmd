...
grids <- create_grids(spatial_object = habs, 
                      # spat_buffer_distance, # could I add the buffer distance to the spatial object which is getting gridded up - this could allow for buffered regions on the edges not to be cropped. However, it might not work because there wouldn't be any objects in the external grids to buffer? TEST
                      grid_size = 100000, 
                      grid_buffer_size = 100000)
...
# grids$buffered_grid_sf <- st_sf(grids$buffered_grid_sf)
# grids$buffered_grid_sf$pid <- 1:nrow(grids$buffered_grid_sf)
# 
# grids$central_grid_sf <- st_sf(grids$central_grid_sf)
# grids$central_grid_sf$pid <- 1:nrow(grids$central_grid_sf)
# 
# ggplot() + 
#   # geom_sf(data = habs) + 
...
# try converting the output polygon to raster
## can't work out how to do this!!
hb <- poly_to_rast(obj = hab_overlaps, 
                       field_val = hab_overlaps$n_overlaps, 
                       resolution = c(5000,5000), 
                       rast_extent = terra::ext(hab_overlaps), 
                       layer_names = NULL)

plot(terra::mosaic(hb, fun = "max"))
plot(overs$habitat_connectivity_raster)


### as long as the grid size is > the buffer size then it works reasonaly well, 
...
######## THIS WORKS NOW NEED TO INCLUDE IT IN FUNCTION!!!!!!!

#### from within the habitat_gridded function - THIS WORKS
plot(mosaic(overlap_hab[[1]],overlap_hab[[3]], fun = "max"))
overs <- sprc(overlap_hab)
plot(mosaic(overs, fun = "max"))
