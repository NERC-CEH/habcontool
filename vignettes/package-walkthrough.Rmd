...
create_grids <- function(spatial_object,
  ...
  ## can I just feed the extents straight to the function?
  
  # get the x-y min max values to define our central region of interest
  central_area_boxes <- do.call(rbind, lapply(1:length(spat_grid), FUN = function(x) {
    get_poly_extremes(spat_grid[x])
  }))
  
  # get the x-y min max values to define our large cropping area
  large_area_boxes <- do.call(rbind, lapply(1:length(spat_grid_buff), FUN = function(x) {
...
df <- st_as_sf(data.frame(spat_grid, id = seq(1:length(spat_grid))))
ggplot() + 
  geom_sf(data = habs) + 
  geom_sf(data = df, fill = "transparent", aes(colour = id)) +
  geom_sf(data = spat_grid[1], colour = "red")


grids <- create_grids(spatial_object = habs, grid_size = 50000, grid_buffer_size = 50000)
